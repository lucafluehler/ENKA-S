...
class CollisionModelGeneratorTest : public ::testing::Test {
protected:
    ...
    void SetUp() override {
        ...
        settings.particle_count_1 = 100;
        settings.sphere_radius_1 = 10.0;
        settings.total_mass_1 = 1.0;
...
TEST_F(CollisionModelGeneratorTest, CreateSystem) {
    ...
    EXPECT_EQ(system.velocities.size(), settings.particle_count_1 + settings.particle_count_2);
    EXPECT_EQ(system.masses.size(), settings.particle_count_1 + settings.particle_count_2);

    // Test position and velocity distributions
    // ...

    const double total_mass = std::accumulate(system.masses.begin(), system.masses.end(), 0.0);
    EXPECT_NEAR(total_mass, settings.total_mass_1 + settings.total_mass_2, 1e-6);
}
